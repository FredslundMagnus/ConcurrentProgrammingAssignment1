...
public class Search {
    ...
    static int ntasks = 10000; // No. of tasks
    ...
    public static void main(String[] argv) {
        try {
            ...
            System.out.printf("\nFile=%s, pattern='%s'\nntasks=%d, nthreads=%d, warmups=%d, runs=%d\n", fname,
                    new String(pattern), ntasks, nthreads, warmups, runs);
            ...
            /*
             * Run a couple of times on engine for loading all classes and cache warm-up
             */
            ...
            for (int i = 0; i < ntasks; i++) {
                double n_pr_task = (double) (len - pattern.length + 1) / (double) (ntasks);
                taskList.add(new SearchTask(text, pattern, (int) (i * n_pr_task),
                        (int) ((i + 1) * n_pr_task + pattern.length - 1)));
            }
